...
def AfterToday():
    message = 'You can only search for rides on today\'s date or after.'

    def _AfterToday(form, field):
...
class SearchFormFactory(FlaskForm):
    ...
    date = DateField("Departure Date:", validators=[InputRequired(message='Please enter desired departure date'), AfterToday()], format='%Y-%m-%d')
